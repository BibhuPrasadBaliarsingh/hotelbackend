...
const rooms = [
  ...
  },
  {
    ...
    type: 'Double',
    price: 129,
    description: 'Serene double room overlooking our lush tropical gardens. Perfect for a relaxing retreat with all modern comforts and elegant décor.',
    images: [
      'https://images.unsplash.com/photo-1566665797739-1674de7a421a?w=800',
      'https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?w=800'
    ],
...
async function seed() {
  try {
    ...
    // Create admin
    const admin = await User.create({ name: 'Admin User', email: 'admin@hotel.com', password: 'admin123', role: 'admin', phone: '+1 555-0000' });
    // Create test user
    const user = await User.create({ name: 'John Doe', email: 'user@hotel.com', password: 'user123', role: 'user', phone: '+1 555-1234' });
    ...
    const lastMonthEnd = new Date(); lastMonthEnd.setDate(lastMonthEnd.getDate() - 25);

    await Booking.create([
      { user: user._id, room: createdRooms[0]._id, checkIn: lastMonth, checkOut: lastMonthEnd, guests: { adults: 1, children: 0 }, totalNights: 5, pricePerNight: 89, totalAmount: 445, status: 'completed', paymentStatus: 'paid' },
      { user: user._id, room: createdRooms[2]._id, checkIn: nextWeek, checkOut: new Date(nextWeek.getTime() + 3 * 86400000), guests: { adults: 2, children: 0 }, totalNights: 3, pricePerNight: 249, totalAmount: 747, status: 'confirmed', paymentStatus: 'paid' },
    ]);
